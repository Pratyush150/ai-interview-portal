...
def send_lead_notification(lead: dict) -> dict:
    ...
    sales_to = os.getenv("SALES_NOTIFY_EMAIL", "sales@apertureai.com")
    subject = f"[ApertureAI] New lead: {lead.get('company_name') or lead.get('contact_name')}"
    lines = [
...
def send_owner_setup_email(
    ...
    subject = f"Welcome to ApertureAI — set up {company_name}"
    body_text = (
        f"Hi {greeting},\n\n"
        f"Your ApertureAI workspace for {company_name} is ready.\n\n"
    ...
    <div style="font-family: Arial, sans-serif; max-width: 560px; margin: 0 auto; padding: 24px;">
        <h2 style="color:#4f46e5;">Welcome to ApertureAI</h2>
        <p>Hi {greeting},</p>
        <p>Your ApertureAI workspace for <strong>{company_name}</strong> is ready.</p>
